...
Let's consider we want to expose the list of US presidents in SData way (this is exactly what is done in "example-app":http://github.com/DanielVartanov/acts_as_sdata/tree/master/example-app/).
...
Also you should to include special routing delimiters which are used in SData paths (remember, we are using Usher, this will _NOT_ work with native Rails Router).
...
_<code>:feed</code> options are fields of Atom feed_
